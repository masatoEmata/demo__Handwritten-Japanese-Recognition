...
train_images = np.zeros([kanji * each_samples, rows, cols], dtype=np.float32)

arr = np.arange(kanji)
train_labels = np.repeat(arr, each_samples)

# 4 characters were actually hiragana, so delete these 4 extras
for i in range( (kanji+4) * each_samples):
    if int(i/each_samples) != 88 and int(i/each_samples) != 219 and int(i/each_samples) != 349 and int(i/each_samples) != 457:
        if int(i/each_samples) < 88:
            train_images[i] = skimage.transform.resize(kan[i], (rows, cols))
        if int(i/each_samples) > 88 and int(i/each_samples) < 219:
            train_images[i-each_samples] = skimage.transform.resize(kan[i], (rows, cols))
        if int(i/each_samples) > 219 and int(i/each_samples) < 349:
            train_images[i-320] = skimage.transform.resize(kan[i], (rows, cols))
        if int(i/each_samples) > 349 and int(i/each_samples) < 457:
            if int(i/each_samples) > 457:
